...
class QUBE:
    def __init__(self, port, baudrate):
        ...
        self.output = self.neutral_output()
        self.input = []
    ...
    def neutral_output():
        return [
            ...
            0,  # blue msb - 6
            0,  # blue lsb - 7
            MOTOR_NEUTRAL_COMMAND >> 8,  # motor msb - 8
            MOTOR_NEUTRAL_COMMAND & 0xFF,  # motor lsb - 9
        ]
    ...
    def find_port(port):
        ...
        for available_port in ports:
            if available_port.vid == 0x16C0 and available_port.pid == 0x0483:
                print(f"Using Teensy/QUBE port: {available_port.device}")
                return available_port.device

        for available_port in ports:
            if available_port.device.startswith(("/dev/ttyACM", "/dev/ttyUSB")):
                print(f"Using serial port: {available_port.device}")
                return available_port.device

        if port and port != "auto":
            return port

        raise serial.SerialException("No Teensy/QUBE serial port found")
    ...
    def setMotorSpeed(self, speed):
        speed = constrain(speed, -MAX_MOTOR_SPEED_COMMAND, MAX_MOTOR_SPEED_COMMAND)
        speed += 999
